...
void Read_Memory(u32 Memory_Offset, u32 Transfer_Length)
{
  ...
  if (TransferState == TXFR_ONGOING )
  {
    ...
    {
      Clash = 1;
      ExtFlash_PageRD(Data_Buffer, Offset, Mass_Block_Size);           
...
void Write_Memory (u32 Memory_Offset, u32 Transfer_Length)
{
  ...
  if (TransferState == TXFR_ONGOING )
  {
    ...
    if (!(W_Length % Mass_Block_Size))
    {
      Counter = 0;
      Clash = 1;
      ExtFlash_PageWR(Data_Buffer, W_Offset - Mass_Block_Size); 
//#ifndef W25Q64BV   
      if(flash_mode==FLASH_2M)
        ExtFlash_PageWR(Data_Buffer + EXT_FLASH_PageSize, W_Offset - EXT_FLASH_PageSize);  
//#endif      
    }
