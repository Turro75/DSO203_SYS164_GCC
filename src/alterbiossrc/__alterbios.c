...
#define STATUS(x) Display_Str(8*8, 110, RGB(0,255,0), 0, (u8*)(VERSION ": " x "                 "))
...
u8 __patch_OpenFileWr(u8 *buffer, u8 *filename, u16 *cluster, u32 *diraddr)
{
    ...
    FRESULT r = __f_open(&g_file, fname, FA_WRITE | FA_CREATE_ALWAYS);
...
u8 __patch_OpenFileRd(u8 *buffer, u8 *filename, u16 *cluster, u32 *diraddr)
{
    ...
    FRESULT r = __f_open(&g_file, fname, FA_READ | FA_OPEN_EXISTING);
    *cluster = g_file.sclust;
    return ERR(r);
}
...
u8 __patch_ReadFileSec(u8* buffer, u16* cluster)
{
    ...
    unsigned rcount;
    FRESULT r = __f_read(&g_file, buffer, fatfs.ssize, &rcount);
...
{
    unsigned wcount;
    FRESULT r = __f_write(&g_file, buffer, fatfs.ssize, &wcount);
...
{
    FRESULT r = __f_lseek(&g_file, length);
    if (r != FR_OK)
        return ERR(r);
