...
#else
	res = __f_sync(fp);		/* Flush cached data */
#if _FS_LOCK
...
FRESULT __f_getfree (
	...
	FATFS **__fatfs		/* Pointer to pointer to corresponding file system object to return */
)
{
	FRESULT res;
	FATFS *fs;
	...
	/* Get drive number */
	res = chk_mounted(&path, __fatfs, 0);
	fs = *__fatfs;
	if (res == FR_OK) {
